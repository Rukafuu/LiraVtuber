...
from lira_core.memory.knowledge_graph import LiraKnowledgeGraph
from lira_core.memory.memory import memory as SQLiteMemory
from lira_core.memory.rag_engine import LiraRAGEngine
from lira_core.utils.lira_tags import sanitize_history_message
...
class LiraMemoryManager:
    def __init__(
        self,
        db_path: str = "data/lira_memory.db",
        *,
        enable_chroma: bool | None = None,
        sync_graph_to_rag: bool = True,
        defer_graph_sync: bool = False,
    ):
        ...
        self.rag = LiraRAGEngine(enable_chroma=enable_chroma)

        # Camada 3: Logica (Grafo de Conhecimento)
        self.graph = LiraKnowledgeGraph()

        # Sincroniza grafo -> RAG (pesado com Chroma; pode ir para thread)
        if sync_graph_to_rag:
            if defer_graph_sync:
                threading.Thread(
                    target=self._sincronizar_grafo_rag,
                    daemon=True,
                    name="LiraGraphRagSync",
                ).start()
            else:
