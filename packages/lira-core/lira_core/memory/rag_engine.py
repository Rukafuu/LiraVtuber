...
class LiraRAGEngine:
    ...
    def __init__(
        self,
        persist_directory: str = "data/memory/chroma_db",
        ...
        self._chroma_thread: threading.Thread | None = None

        if enable_chroma is None:
            enable_chroma = os.getenv("LIRA_RAG_CHROMA", "1").lower() in ("1", "true", "yes")
        self._chroma_enabled = bool(enable_chroma)
        ...
        if self._chroma_enabled:
            logger.info("[RAG ENGINE] ChromaDB sera carregado em segundo plano: %s", self.os_path)
            self.start_chroma_background()
        else:
            logger.info(
                "[RAG ENGINE] Modo lexical (sem Chroma no startup). "
                "Busca vetorial via API/HUD usa fallback ate LIRA_RAG_CHROMA=1."
            )

    def start_chroma_background(self):
        """Dispara carga do Chroma + SentenceTransformer sem bloquear o processo principal."""
        with self._chroma_lock:
            if self._chroma_thread and self._chroma_thread.is_alive():
                return
            if self.collection is not None:
                return
            self._chroma_enabled = True
            self._chroma_thread = threading.Thread(target=self._init_chroma, daemon=True, name="LiraChromaInit")
    ...
    def _init_chroma(self):
        try:
            ...
            logger.info("[RAG ENGINE] ✅ ChromaDB pronto em segundo plano.")
        except Exception as exc:
            self.client = None
            self.collection = None
            self.embedding_fn = None
            logger.warning(
                "[RAG ENGINE] ChromaDB/embeddings indisponiveis. Usando fallback lexical. Motivo: %s",
                exc,
            )
    ...
    def get_all_memories(self) -> List[Dict[str, Any]]:
        """Lista memorias para o editor da HUD (fallback + Chroma quando pronto)."""
        ...
        if self.collection is not None:
            try:
                ...
                logger.error("[RAG ENGINE] Erro em get_all_memories: %s", exc)
    ...
    def delete_memory(self, mem_id: str) -> bool:
        ...
        if self.collection is not None:
            try:
                ...
                logger.error("[RAG ENGINE] Erro em delete_memory: %s", exc)
